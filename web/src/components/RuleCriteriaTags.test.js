...
describe("RuleCriteriaTags", () => {
  ...
  it("renders process matching criteria tags (process_name, process_path, package_name, user)", () => {
    const processRule = {
      outbound: "proxy",
      process_name: ["msedge.exe", "chrome.exe"],
      process_path: ["C:\\Program Files\\Edge\\msedge.exe"],
      process_path_regex: [".*edge.*"],
      package_name: ["com.microsoft.emmx"],
      user: ["root"],
    };
    const wrapper = mount(RuleCriteriaTags, {
      props: { rule: processRule, type: "route" },
      global: { stubs: globalStubs },
    });
    expect(wrapper.text()).toContain("Process: msedge.exe, chrome.exe");
    expect(wrapper.text()).toContain(
      "Process Path: C:\\Program Files\\Edge\\msedge.exe",
    );
    expect(wrapper.text()).toContain("Process Path Regex: .*edge.*");
