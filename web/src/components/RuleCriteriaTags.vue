<template>
  <div class="rule-criteria-tags">
    ...
    <span
      v-if="rule.process_name"
      class="criteria-tag"
      :class="{
        'duplicate-tag': isDuplicate('process_name', rule.process_name),
      }"
    >
      Process: {{ formatArray(rule.process_name) }}
    </span>
    <span v-if="rule.process_path" class="criteria-tag">
      Process Path: {{ formatArray(rule.process_path) }}
    </span>
    <span v-if="rule.process_path_regex" class="criteria-tag">
      Process Path Regex: {{ formatArray(rule.process_path_regex) }}
    </span>
    <span v-if="rule.package_name" class="criteria-tag">
      Package: {{ formatArray(rule.package_name) }}
    </span>
    <span v-if="rule.user" class="criteria-tag">
      User: {{ formatArray(rule.user) }}
    </span>
    ...
    <template v-if="rule.type === 'logical' && Array.isArray(rule.rules)">
      <template v-for="(sub, sidx) in rule.rules" :key="sidx">
        ...
        <span
          v-if="sub.process_name"
          class="criteria-tag"
          :class="{
            'duplicate-tag': isDuplicate('process_name', sub.process_name),
          }"
        >
          Process: {{ formatArray(sub.process_name) }}
        </span>
        <span v-if="sub.process_path" class="criteria-tag">
          Process Path: {{ formatArray(sub.process_path) }}
        </span>
        <span v-if="sub.process_path_regex" class="criteria-tag">
          Process Path Regex: {{ formatArray(sub.process_path_regex) }}
        </span>
        <span v-if="sub.package_name" class="criteria-tag">
          Package: {{ formatArray(sub.package_name) }}
        </span>
        <span v-if="sub.user" class="criteria-tag">
          User: {{ formatArray(sub.user) }}
        </span>
        <span
