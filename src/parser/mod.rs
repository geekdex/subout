...
fn parse_url_to_outbound(url: &Url, force_https: bool) -> Option<Outbound> {
    ...
    match scheme {
        "socks" | "socks5" => Some(Outbound::Socks(SocksOutbound {
            tag,
            server: host,
            server_port: port,
            username,
        ...
        "http" | "https" | _ if scheme == "http" || scheme == "https" || force_https => {
            ...
            let tls_config = if is_secure {
                Some(TlsConfig {
                    enabled: true,
                    server_name: extract_sni(&host, &params),
...
mod tests {
    ...
    #[test]
    fn test_parse_https_node_defaults_sni_to_server() {
        let line = "https://dXNlcjpwYXNzQGV4YW1wbGUuY29tOjQ0My8=#MyNode";
        let outbound = parse_line(line).expect("Should parse https node");
        if let Outbound::Http(http) = outbound {
            assert_eq!(http.server, "example.com");
            let tls = http.tls.expect("TLS should be enabled for https");
            assert_eq!(tls.server_name, Some("example.com".to_string()));
        } else {
            panic!("Expected Http outbound");
        }
    }

    #[test]
    fn test_parse_socks5_node() {
        let line = "socks5://user:pass@socks.example.com:443#SocksNode";
        let outbound = parse_line(line).expect("Should parse socks5 node");
        if let Outbound::Socks(socks) = outbound {
            assert_eq!(socks.server, "socks.example.com");
            assert_eq!(socks.server_port, 443);
        } else {
