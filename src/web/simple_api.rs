...
pub async fn get_simple_config(
    ...
    let generated = simple_config::generate_simple_singbox_config(&conn, &cfg)
...
pub async fn save_simple_config(
    ...
    let log = generated.get("log").cloned().unwrap_or(serde_json::json!({}));
    let dns = generated.get("dns").cloned().unwrap_or(serde_json::json!({}));
    let inbounds = generated.get("inbounds").cloned().unwrap_or(serde_json::json!([]));
    let outbounds = generated.get("outbounds").cloned().unwrap_or(serde_json::json!([]));
    let route = generated.get("route").cloned().unwrap_or(serde_json::json!({}));
    let experimental = generated.get("experimental").cloned().unwrap_or(serde_json::json!({}));

    if let Err(err_msg) = validate_config_with_singbox(&log, &dns, &inbounds, &outbounds, &route, &experimental) {
        return Err((StatusCode::BAD_REQUEST, format!("配置校验未通过: {}", err_msg)));
    }

    simple_config::save_simple_config(&conn, &payload.config)
    ...
    if payload.apply.unwrap_or(false) {
        ...
        if let Err(e) = state.service_manager.restart_with_sudo(&generated, sudo_pass.as_deref()).await {
            return Err((StatusCode::INTERNAL_SERVER_ERROR, format!("配置已保存，但重启服务失败: {}", e)));
        }
    }
